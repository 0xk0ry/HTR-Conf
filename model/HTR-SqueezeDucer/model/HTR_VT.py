...
class RNNTHead(nn.Module):
    """
    Prediction net: Embedding -> (LSTM or GRU) -> proj
    Joint net:      tanh(W_enc h_t + W_pred g_u + b) -> linear -> vocab_with_blank
    """

    def __init__(self, vocab_size: int, blank_id: int, d_model: int,
                 pred_dim: int = 512, pred_layers: int = 1, joint_dim: int = 512,
        ...
        if rnn_type.lower() == "gru":
            self.rnn = nn.GRU(d_model, pred_dim,
                              num_layers=pred_layers, batch_first=True)
        else:
            self.rnn = nn.LSTM(d_model, pred_dim,
                               num_layers=pred_layers, batch_first=True)

        self.pred_proj = nn.Linear(pred_dim, joint_dim)
        self.enc_proj = nn.Linear(d_model, joint_dim)
        self.joint_act = nn.Tanh()
        # includes blank_id inside [0..vocab_size-1]
        self.joint_out = nn.Linear(joint_dim, vocab_size)
        self.dropout = nn.Dropout(dropout)

    def prediction(self, y_in: torch.Tensor):
        """
    ...
    def joint(self, f_t: torch.Tensor, g_u: torch.Tensor):
        """
        f_t: [B, T, d_model]  (encoder)
        g_u: [B, U, pred_dim] (prediction)
        returns logits: [B, T, U, V]
        """
        f = self.enc_proj(f_t)                # [B, T, J]
        g = self.pred_proj(g_u)               # [B, U, J]
        # Broadcast sum to [B, T, U, J]
        f = f.unsqueeze(2)
        g = g.unsqueeze(1)
        z = self.joint_act(f + g)             # [B, T, U, J]
        z = self.dropout(z)
        return self.joint_out(z)              # [B, T, U, V]
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        self.nb_cls = nb_cls                   # same vocab you used for CTC
        # <- pick an index for blank (last token works well)
        self.blank_id = nb_cls - 1
    ...
    # Small helper to expose encoder features for RNNT path
    def encode(self, x, use_masking=False, mask_mode: str = "mms", mask_ratio: float = 0.0, max_span_length: int = 8):
        """
        Returns encoder features [B, T, D] without classification head.
        This mirrors forward_features and is used by RNNT predictor/joint.
        """
    ...
    @torch.no_grad()
    def greedy_decode_rnnt(self, x, max_U=200):
        """
        Simple greedy decoding for debugging (no LM).
        """
        self.eval()
        with torch.no_grad():
            f = self.encode(x, use_masking=False)                # [B, T, D]
            B, T, _ = f.shape
            y = torch.full((B, 1), fill_value=0, dtype=torch.long, device=f.device)  # assume 0=BOS
            out = [[] for _ in range(B)]

            for t in range(T):
                g = self.rnnt_head.prediction(y)                 # [B, U, P]
                logits = self.rnnt_head.joint(f[:, t:t+1], g)    # [B, 1, U, V]
                step = logits[:, 0, -1]                          # [B, V], use last u
                tok = step.argmax(-1)                            # [B]
                for b in range(B):
                    if tok[b].item() != self.blank_id and len(out[b]) < max_U:
                        out[b].append(tok[b].item())
                        y = torch.cat([y, tok.unsqueeze(1)], dim=1)
        return out

    def forward_rnnt(self, x, y_in):
        """
        x:   images -> encoder features [B, T, D]
        y_in: int64 targets *with BOS prepended* [B, U]
        returns logits for loss: [B, T, U, V]
        """
        f = self.encode(x, use_masking=True)           # keep your masking if you want
        g = self.rnnt_head.prediction(y_in)
        logits = self.rnnt_head.joint(f, g)            # [B, T, U, V]
        return logits, f.shape[1]

def create_model(nb_cls, img_size, mlp_ratio=4, architecture: str = "squeezeformer", conv_kernel_size=7, se_ratio: float = 0.25, **kwargs):
