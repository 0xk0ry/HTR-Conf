...
class MobileViTBlock(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for j, transformer_layer in enumerate(self.global_rep):
            ...
            if j == 0:
                # Apply positional encoding generator (PEG) to the token sequence.
                # Use `patches` (BP x N x C) not the original image tensor `x`.
                patches = self.pos_pe(
                    patches, num_patch_h, num_patch_w)  # PEG here
