...
def _downsample_mask(self, m, k=3, s=2, p=1):
    # m: [B, N, 1] with 1=keep, 0=masked
    inv = 1.0 - m.transpose(1, 2)      # [B,1,N]
    # any masked in window -> masked (max over window)
    inv_ds = F.max_pool1d(inv, kernel_size=k, stride=s,
                          padding=p, ceil_mode=True)
    m_ds = 1.0 - inv_ds                 # back to keep=1/0
    return m_ds.transpose(1, 2)         # [B, N//s, 1]


def _upsample_mask(self, m_ds, target_len, mode='nearest'):
    # m_ds: [B, N_lo, 1]
    up = F.interpolate(m_ds.transpose(1, 2), size=target_len,
                       mode=('linear' if mode == 'linear' else 'nearest'),
                       align_corners=False if mode == 'linear' else None)
    # binarize; keep hard edges so skip gate is crisp
    up = (up > 0.999).float()
    return up.transpose(1, 2)           # [B, target_len, 1]
...
class HTR_ConvText(nn.Module):
    ...
    def forward_features(self, x, use_masking=False,
                         mask_mode="span", mask_ratio=0.5, block_span=4, max_span_length=8):
        ...
        if use_masking:
            if mask_mode == "random":
                keep_mask_1d = self.mask_random_1d(
                    x, mask_ratio).float()   # [B,N]
            elif mask_mode == "block":
                keep_mask_1d = self.mask_block_1d(
                    x, mask_ratio, block_span).float().squeeze(-1)
            else:
                keep_mask_1d = self.mask_span_1d(
                    x, mask_ratio, max_span_length).float().squeeze(-1)
            # [B,N,1]
            keep_mask = keep_mask_1d.unsqueeze(-1)
            x = keep_mask * x + (1.0 - keep_mask) * \
                self.mask_token.expand(B, N, C)

        # --- U-path with mask propagation & gated skip ---
        skip_hi = None
        # may be None
        keep_hi = keep_mask
        keep_lo = None
        ...
        for i, blk in enumerate(self.blocks, 1):
            ...
            if i == self.down_after:
                ...
                skip_hi = x
                if (x.size(1) % 2) == 1:
                    # pad features AND mask consistently
                    x = torch.cat([x, x[:, -1:, :]], dim=1)
                    if keep_hi is not None:
                        keep_hi = torch.cat(
                            [keep_hi, keep_hi[:, -1:, :]], dim=1)
                # [B,N_lo,C]
                x = self.down1(x)

                if keep_hi is not None:
                    # mirror downsample on mask
                    k = self.down1.dw.kernel_size[0]
                    s = self.down1.dw.stride[0]
                    p = self.down1.dw.padding[0]
                    keep_lo = self._downsample_mask(
                        keep_hi, k=k, s=s, p=p)   # [B,N_lo,1]

            if i == self.up_after:
                assert skip_hi is not None
                x = self.up1(x, target_len=skip_hi.size(
                    1))                   # [B,N_hi,C]

                if keep_lo is not None:
                    keep_up = self._upsample_mask(keep_lo, target_len=skip_hi.size(1),
                                                  # [B,N_hi,1]
                                                  mode=self.up1.mode)
                    # gate skip so masked positions don't get clean info from skip
                    x = x + skip_hi * keep_up
                    # update "current" mask
                    keep_hi = keep_up
                else:
        ...
        x = self.norm(x)
        # final mask (after geometry) for downstream losses
        final_masked_positions_1d = None
        if keep_hi is not None:
            final_masked_positions_1d = (1.0 - keep_hi.squeeze(-1)).clamp(0, 1)
        return x, final_masked_positions_1d
