...
def compute_losses(
    ...
    if sgm_head is not None and feats is not None:
        ...
        # CTC-guided attention mask (Gaussian over top-k CTC confidence anchors)
        with torch.no_grad():
            # preds: [B, N, V_ctc] -> max probability over vocab per time step
            ctc_probs = preds.softmax(-1).max(-1).values  # [B, N]
            N = ctc_probs.size(1)
            k = min(getattr(args, 'ctc_mask_topk', 8), N)
            topk_idx = torch.topk(ctc_probs, k=k, dim=1).indices  # [B, k]

            pos = torch.arange(N, device=preds.device).view(1, 1, N)  # [1,1,N]
            anchors = topk_idx.unsqueeze(-1)  # [B,k,1]
            dist2 = (pos - anchors) ** 2      # [B,k,N]
            sigma = getattr(args, 'ctc_mask_sigma', None)
            if sigma is None:
                sigma = max(1.0, N / 16.0)
            gauss = torch.exp(-dist2.float() / (2.0 * (float(sigma) ** 2)))  # [B,k,N]
            vis_1d = gauss.max(dim=1).values  # [B,N] in [0,1]

            # Tile per target position L
            L = left_ctx.size(1)
            vis_mask = vis_1d.unsqueeze(1).expand(-1, L, -1)  # [B, L, N]
