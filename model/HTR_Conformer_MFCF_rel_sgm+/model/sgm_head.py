...
class SGMHead(nn.Module):
    ...
    def __init__(self, d_vis, vocab_size_sgm, d_txt=256, sub_str_len=5, num_heads=8, p_drop=0.1):
        ...
        self.ctx_conv = nn.Sequential(
            nn.Conv1d(d_txt, d_txt, kernel_size=3, padding=1, groups=d_txt, bias=True),  # depthwise
            nn.SiLU(),
            nn.Conv1d(d_txt, d_txt, kernel_size=1, bias=True),                          # pointwise
        )
        self.ctx_gate = nn.Linear(d_txt, 1)  # gated pooling weights
    ...
    def _context_to_query(self, ctx_ids, dir_token):  # ctx_ids: [B, L, S]
        ...
        E = E.transpose(2, 3)                        # [B, L, d_txt, S]
        E = self.ctx_conv(E.reshape(-1, E.size(2), E.size(3)))   # [(B*L), d_txt, S]
        E = E.transpose(1, 2).reshape(-1, self.sub_str_len, E.size(1))  # [(B*L), S, d_txt]
        g = torch.softmax(self.ctx_gate(E).squeeze(-1), dim=1)   # [(B*L), S]
        q = (g.unsqueeze(-1) * E).sum(dim=1).view(ctx_ids.size(0), ctx_ids.size(1), -1)
        q = q + dir_token
        q = self.txt_proj(q)
